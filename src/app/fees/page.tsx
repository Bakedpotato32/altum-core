...
export default function StudentFeeDiary() {
  ...
  const shieldConfig: Record<string, any> = {
    cleared: { bg: 'bg-emerald-500 border-emerald-400', icon: <ShieldCheck size={120} className="absolute right-[-20px] bottom-[-20px] text-white/10 -rotate-12" />, title: t('cleared'), text: 'text-emerald-100', subtitle: `Till: ${student.paid_till}` },
    warning: { bg: 'bg-yellow-500 border-yellow-400', icon: <AlertCircle size={120} className="absolute right-[-20px] bottom-[-20px] text-black/10 -rotate-12" />, title: t('dueSoon'), text: 'text-yellow-900', subtitle: `${t('pendingFor')} 1 ${t('month')}` },
    alert: { bg: 'bg-orange-500 border-orange-400', icon: <AlertTriangle size={120} className="absolute right-[-20px] bottom-[-20px] text-white/10 -rotate-12" />, title: t('overdue'), text: 'text-orange-100', subtitle: `${t('pendingFor')} 2 ${t('months')}` },
    danger: { bg: 'bg-red-500 border-red-400', icon: <ShieldAlert size={120} className="absolute right-[-20px] bottom-[-20px] text-white/10 -rotate-12" />, title: t('actionNeeded'), text: 'text-red-100', subtitle: student.paid_till && student.paid_till.toUpperCase() !== 'PENDING' ? `${t('overdueSince')} ${student.paid_till}` : t('contactAdmin') }
  };
    ...
    <div className="min-h-screen bg-transparent p-6 pt-28 pb-40 font-sans text-[var(--text)]">
      <div className="max-w-md mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-[3px] mt-1 italic opacity-60">{t('financeNode')}</p>
        </div>
        ...
        <div className={`p-8 rounded-[35px] border relative overflow-hidden shadow-lg transition-colors ${activeShield.bg}`}>
          ...
          <div className="relative z-10">
            <h3 className={`text-[10px] font-black uppercase tracking-[4px] mb-2 ${activeShield.text} opacity-80`}>{t('currentStatus')}</h3>
        ...
        <div className="pt-6">
          <div className="flex items-center gap-2 mb-6 ml-2">
             ...
             <h2 className="text-[10px] font-black text-zinc-500 uppercase tracking-[4px]">{t('paymentHistory')}</h2>
          </div>
          ...
          {history.length > 0 ? (
            ...
            <div className="text-center py-16 bg-zinc-500/5 border border-dashed border-[var(--border)] rounded-[40px]">
               <p className="text-[9px] font-black uppercase tracking-widest text-zinc-500 opacity-40 italic px-10 leading-relaxed">{t('noPaymentRecords')}</p>
            </div>
          )}
        </div>
      </div>
