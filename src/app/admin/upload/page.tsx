...
export default function AdminUpload() {
  ...
  useEffect(() => {
    ...
    fetchActiveClasses();
    fetchMaterials(isMaster, assigned);
  }, []);
  ...
  // Fetch Dynamic Subjects whenever the Standard changes
  useEffect(() => {
    if (!standard) {
      setClassSubjects([]);
      return;
    ...
    const fetchSubjects = async () => {
      ...
      const key = getSubjectKey(standard);
      ...
      setSubject(""); // Reset subject selection when class changes
  ...
  const fetchMaterials = async (isMaster: boolean, assigned: string | null) => {
    setFetching(true);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      fetchMaterials(isMasterAdmin, standard);
          ...
          <div className="flex items-center gap-4 relative">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[4px] mt-2 flex items-center gap-2">
                <span className="w-6 h-[1px] bg-zinc-400 block"></span> {isMasterAdmin ? "Master Cloud Node" : `Class ${standard} Library`}
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleUpload} className="bg-[var(--card)]/90 backdrop-blur-2xl border border-[var(--border)] border-t-4 border-t-green-500 rounded-[35px] p-7 space-y-5 shadow-xl relative overflow-visible">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="relative">
              {!isMasterAdmin && <Lock size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-green-600 z-10" />}
              <select 
                required 
                disabled={!isMasterAdmin}
                value={standard} 
                onChange={(e) => setStandard(e.target.value)} 
                className={`w-full bg-[var(--background)] border border-[var(--border)] rounded-[20px] p-5 text-xs font-black uppercase text-[var(--text)] outline-none appearance-none focus:border-green-500 focus:ring-4 focus:ring-green-500/10 transition-all shadow-inner ${!isMasterAdmin ? 'pl-10 opacity-70' : ''}`}
              >
                <option value="">{!isMasterAdmin ? `Class ${standard}` : "Standard"}</option>
                {activeClasses.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>

            <div className="relative">
              {loadingSubjects && <Loader2 size={14} className="absolute right-4 top-1/2 -translate-y-1/2 text-green-500 animate-spin z-10" />}
              <select required value={subject} onChange={(e) => setSubject(e.target.value)} disabled={!standard || classSubjects.length === 0} className="w-full bg-[var(--background)] border border-[var(--border)] rounded-[20px] p-5 text-xs font-black uppercase text-[var(--text)] outline-none appearance-none focus:border-green-500 focus:ring-4 focus:ring-green-500/10 transition-all shadow-inner disabled:opacity-50">
                <option value="">{classSubjects.length === 0 && standard ? "No Subjects" : "Subject"}</option>
              ...
              {allMaterials.length === 0 ? (
                ...
                allMaterials.map((file) => (
                  <div key={file.id} className="p-5 bg-[var(--card)]/60 backdrop-blur-md border border-[var(--border)] border-l-4 border-l-green-500 rounded-[24px] flex items-center justify-between shadow-sm group active:scale-[0.98] hover:shadow-md transition-all">
                    ...
                    <button onClick={async () => { if(confirm("Delete this document?")) { await supabase.from('materials').delete().eq('id', file.id); fetchMaterials(isMasterAdmin, standard); } }} className="w-8 h-8 shrink-0 rounded-full bg-[var(--background)] border border-[var(--border)] flex items-center justify-center text-zinc-400 hover:text-red-500 hover:border-red-500/30 hover:bg-red-500/5 active:scale-90 transition-all">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))
