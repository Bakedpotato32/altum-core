...
import { Calculator, Sparkles, ChevronRight, RotateCcw, Target, Zap, CheckCircle2 } from 'lucide-react';

export default function AlgebraSolver() {
  const [input, setInput] = useState("2x² + 6x + 9 = 3x² - 8x + 27");
  const [steps, setSteps] = useState<{ eq: string, exp: string }[]>([]);
  ...
  const parseSide = (str: string) => {
    ...
    // Normalize input: remove spaces, handle subtraction as adding negatives, normalize x^2
  ...
  const formatTerm = (coeff: number, v: string) => {
    ...
    let s = coeff < 0 ? '-' : '+';
    if (v !== '' && c === 1) c = ''; // display 'x' instead of '1x'
    return `${s} ${c}${v}`;
  };
  ...
  const getTargetText = (task: string, r: any, l: any) => {
    if (task === 'move_x2') return `Shift ${Math.abs(r.x2)}x² to the left side`;
    if (task === 'move_x') return `Shift ${Math.abs(r.x)}x to the left side`;
    if (task === 'move_c_to_lhs') return `Set equation to 0 by moving ${Math.abs(r.c)}`;
    if (task === 'move_c_to_rhs') return `Isolate x by moving ${Math.abs(l.c)} to the right`;
    if (task === 'solve_quad') return `Apply the Quadratic Formula`;
    if (task === 'solve_linear') return `Calculate final value of x`;
    return null;
  };
  ...
  const initTower = () => {
    ...
    let initialEq = printEq(lhs, rhs);
    let initialSteps = [{ eq: initialEq, exp: "Equation scanned and like terms combined." }];
  ...
  const solveNextStep = () => {
    ...
    if (task === 'move_x2') {
      ...
      nL.x2 -= val;
      nR.x2 = 0;
      exp = `${val > 0 ? 'Subtract' : 'Add'} ${Math.abs(val)}x² ${val > 0 ? 'from' : 'to'} both sides to group squared terms.`;
      ...
      nL.x -= val;
      nR.x = 0;
      exp = `${val > 0 ? 'Subtract' : 'Add'} ${Math.abs(val)}x ${val > 0 ? 'from' : 'to'} both sides.`;
      resultEq = printEq(nL, nR);
    } 
      ...
      nL.c -= val;
      nR.c = 0;
      exp = `${val > 0 ? 'Subtract' : 'Add'} ${Math.abs(val)} ${val > 0 ? 'from' : 'to'} both sides to set the equation to zero.`;
      resultEq = printEq(nL, nR);
    } 
      ...
      nR.c -= val;
      nL.c = 0;
      exp = `${val > 0 ? 'Subtract' : 'Add'} ${Math.abs(val)} ${val > 0 ? 'from' : 'to'} both sides to isolate x.`;
      resultEq = printEq(nL, nR);
    } 
      ...
      if (nL.x === 0) {
        ...
        let ans = nR.c / nL.x;
        resultEq = `x = ${Number(ans.toFixed(2))}`;
        exp = `Divide both sides by ${nL.x} to solve for x.`;
      }
    } 
      ...
      let d = (b * b) - (4 * a * c);
      if (d < 0) {
        exp = `Discriminant (b² - 4ac) = ${Number(d.toFixed(2))}. It is negative, so there are no real roots.`;
        resultEq = `No Real Solutions`;
      } else if (d === 0) {
        let root = -b / (2 * a);
        exp = `Applied Quadratic Formula. Discriminant is 0, yielding exactly 1 real root.`;
        resultEq = `x = ${Number(root.toFixed(2))}`;
      } else {
        let r1 = (-b + Math.sqrt(d)) / (2 * a);
        let r2 = (-b - Math.sqrt(d)) / (2 * a);
        exp = `Applied Quadratic Formula: x = (-b ± √(b² - 4ac)) / 2a.`;
        resultEq = `x ≈ ${Number(r1.toFixed(2))} or x ≈ ${Number(r2.toFixed(2))}`;
      }
    }
  ...
  return (
    <div className="relative rounded-3xl bg-card border border-border p-5 overflow-hidden transition-all duration-300 hover:shadow-[0_0_30px_rgba(16,185,129,0.1)] group">
      ...
      <div className="flex items-center gap-2 mb-4 relative z-10">
        ...
        <span className="text-[10px] font-extrabold tracking-[0.2em] uppercase text-emerald-500">
          Smart Solver Engine
        </span>
      </div>
      ...
      <div className="flex gap-2 mb-6 relative z-10">
        <div className="flex-1 relative group/input">
          <input 
            ...
            placeholder="e.g. 2x² + 6x + 9 = 3x² - 8x + 27"
          />
        </div>
        <button 
          onClick={initTower}
          className="bg-emerald-500 hover:bg-emerald-600 text-white p-3 rounded-xl transition-all active:scale-90 shadow-lg shadow-emerald-500/20 flex items-center justify-center"
        >
          ...
          return (
            <div 
              ...
              className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-500 origin-top
                ${isCurrent 
                  ? (isFinal ? 'bg-emerald-500/15 border-emerald-500/40 shadow-[0_0_20px_rgba(16,185,129,0.15)]' : 'bg-emerald-500/10 border-emerald-500/30') 
            ...
            >
              ...
              <div className="flex items-center">
                <span className={`text-xl font-black italic tracking-tight ${isCurrent && isFinal ? 'text-emerald-500' : (isCurrent ? 'text-text' : 'text-text/50')}`}>
                  {step.eq}
                </span>
      ...
      {/* Action Button */}
      <button 
        disabled={steps.length === 0 || isFinal}
        onClick={solveNextStep}
        className={`w-full mt-4 group/btn relative rounded-2xl py-4 flex items-center justify-center gap-2 overflow-hidden transition-all duration-300 active:scale-[0.98]
          ${isFinal 
            ? 'bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.3)] text-white' 
            : 'bg-emerald-500 shadow-[0_4px_0_rgb(4,120,87)] hover:shadow-[0_2px_0_rgb(4,120,87)] hover:translate-y-[2px] active:shadow-none active:translate-y-[4px] text-white'
          }
